...
//! Test: See the sibling `tests` module (`http_client_returns_same_instance`,
//! `strip_service_tier_removes_field`, `build_raw_request_*`) — split out of
//! this file (#2410) to keep the production SLOC cap after adding Fireworks
...
/// The (provider name, env var name) pair to name in the missing-credential
/// error below, keyed off which `ApiEndpoint` the adapter resolved.
///
/// Why: (#2410) `send_raw_completion` is shared by every raw-path adapter
/// (OpenRouter/Generic/Anthropic-fallback, Ollama, Fireworks); the error
/// message must name the provider that actually failed to resolve, not a
/// hard-coded default — Ollama never reaches this branch (its `base_url`
/// contains `localhost`, guarded above), so only OpenRouter-family and
/// Fireworks endpoints are distinguished here today.
/// What: `Fireworks` → `("fireworks", "FIREWORKS_API_KEY")`; every other
/// `AuthSource` this function can observe → the OpenRouter pair (unchanged
/// pre-#2410 behavior).
/// Test: `send_raw_completion_fireworks_missing_key_errors_with_fireworks_name`,
...
pub(super) async fn send_raw_completion(
    ...
    if auth_value.is_empty() && !url.contains("localhost") && !url.contains("127.0.0.1") {
        // #2410: name the ACTUAL provider the adapter resolved to, not a
        // hard-coded "openrouter" — a Fireworks-routed call with no
        // FIREWORKS_API_KEY configured previously reported a misleading
        // "openrouter credential not found" (the wrong provider AND the
        // wrong env var / config-command hint).
        let (provider_name, env_var) = credential_hint(&endpoint.auth_source);
        anyhow::bail!(
            "{provider_name} credential not found (checked process env, .env.local, and the \
             secure store — run `tagent config keys set {provider_name}` or export \
             {env_var}) and adapter supplied no credential"
        );
    }
